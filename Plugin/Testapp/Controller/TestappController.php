 ...
 */
class TestappController extends TestappAppController {
 ...
 */
	function admin_buildnewmobilefiles(){
		...
		$schema =& new CakeSchema(array('name'=>'DsSchema','plugin'=>'Testapp'));
		...
		$alloystr2 .= '};
		';
		$this->bakeMobileAlloyFile("Alloy.Globals.BASEURL = 'http://www.derekstearns.com/appcreator/';".$alloystr, $alloystr2, $alloystr3);
	...
	function admin_downloadmobile(){
		$source_dir = rtrim(getcwd (),'webroot').'Plugin/Testapp/webroot/mobile/';
		$dest_dir =  rtrim(getcwd (),'webroot').'Plugin/Testapp/webroot/';
		$zip_file = 'Testappmobile'.date('Ymd').'.zip';
 
		if($this->Zip($source_dir, $dest_dir.$zip_file)!=false){
	
			$this->redirect('http://www.derekstearns.com/appcreator'.'/testapp/'.$zip_file);
		}
	}
	...
	function bakeMobileAlloyFile($alloystr , $alloystr2, $alloystr3){
		...
		$totalalloystr .= "//:::::::APP CREATOR SYNC OPTIONS:::::::
			...
			$pathx = rtrim(getcwd (),'webroot').'Plugin/Testapp/webroot/mobile/app/';
